...
SQUAD2_DEV_JSONL="/home/qa/data2/tmp/selective_answering/data/g90_seed42/eval_test.jsonl"

OUT_ROOT="${PROJ_ROOT}/results/eval/new2_qwen25_1p5b"
...
SPLITS=(g50_seed42 g40_seed42 g30_seed42 g20_seed42 g10_seed42)
SEEDS=(42)
...
# 하이퍼파라미터
MAX_CTX=3              # SQuAD는 context 1개 고정
MAX_NEW_TOKENS=64
BATCH_SIZE=1
TEMPERATURE=0
TOP_P=0.9
    ...
    for v in "${VARIANTS[@]}"; do
      ...
      for thr in "${THRS[@]}"; do
        for alpha in "${ALPHAS[@]}"; do
          ...
          OUT_JSON="${OUT_ROOT}/${CKPT_TAG}__thr${thr}__a${alpha}.json"
          ts="$(date +%Y%m%d_%H%M%S)"
          LOG_FILE="${LOG_ROOT}/eval_${CKPT_TAG}__thr${thr}__a${alpha}__${ts}.log"

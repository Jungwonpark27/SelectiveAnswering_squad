...
TRAIN_PY="/data2/tmp/selective_answering/src/train.py"

MODEL_TAG="qwen25_1p5b"
...
CKPT_SUBDIR="qwen25_1p5b"  # fallback
...
for split in "${SPLITS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    for v in "${VARIANTS[@]}"; do
      ...
      train_path="/home/qa/data2/tmp/project/data/${split}/${v}_train.jsonl"
      eval_path="/home/qa/data2/tmp/project/data/${split}/eval_val_${v}.jsonl"

      # ckpt/output 경로 규칙
      #out_dir="${CKPT_ROOT}/${split}/${v}_seed${seed}"
      out_dir="/home/qa/data2/tmp/selective_answering/ckpt/qwen25_1p5b_0130/${split}/${v}_seed${seed}"
      ...
      if [[ "${USE_DEEPSPEED}" == "1" ]]; then
        ...
        deepspeed --num_gpus "${NUM_GPUS}" "${TRAIN_PY}" \
          --config "${dump_cfg}" \
          --deepspeed "${DS_CONFIG}" \
        2>&1 | tee "${log_file}"
      else
